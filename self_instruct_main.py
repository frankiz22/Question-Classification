...
if __name__ == "__main__":
    ...
    seed_tasks = [json.loads(l) for l in open(args.seed_tasks_path, "r")][0]
    try :
      seed_instructions = [
          {
    ...
    header = open("Self-instruct/prompts/prompt.txt").read() + "\n###\n"
    request_idx = 0
    while len(machine_instructions) < args.num_instructions_to_generate:
        ...
        with open(args.output_data_path, "a") as fout:
            for inst in instructions:
                ...
                classes = ['translate', 'travel_alert', 'flight_status', 'lost_luggage', 'travel_suggestion', 'carry_on', 'book_hotel']
                if max(rouge_scores) > args.threshold or inst["output"] not in classes:
                    continue
                keep += 1
